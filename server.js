...
app.get('/api/admin/kyc/:userId/document/:docId/view', adminMiddleware, async (req, res) => {
  ...
  const { data: doc } = await supabase
    ...
    .eq('id', docId)
    .single();
  if (!doc || !doc.file_data) return res.status(404).json({ error: 'Document not found or not yet uploaded' });
  /* Return the data URL directly — frontend renders it in an <img> or <iframe> */
  res.json({ url: doc.file_data, filename: doc.filename });
});
