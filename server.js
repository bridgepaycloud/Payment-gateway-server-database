...
app.post('/api/verification/upload', authMiddleware, upload.single('file'), async (req, res) => {
  ...
  const { data: insertedDoc, error } = await supabase.from('verification_documents').upsert({
    user_id: req.user.id,
    document_type,
    filename: safeName,
    file_path: filePath,
  ...
  res.json({ success: true, id: insertedDoc && insertedDoc.id, url: publicUrl, document_type, filename: safeName, file_size: fileSizeKB });
});
...
app.post('/api/admin/kyc/:userId/verified', adminMiddleware, async (req, res) => {
  ...
  await supabase.from('profiles').update({ verification_status: 'verified', updated_at: new Date().toISOString() }).eq('id', uid);
...
app.post('/api/admin/kyc/:userId/rejected', adminMiddleware, async (req, res) => {
  const uid = req.params.userId;
  const { reason } = req.body;
  await supabase.from('profiles').update({ verification_status: 'rejected', updated_at: new Date().toISOString() }).eq('id', uid);
